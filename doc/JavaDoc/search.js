  ...
  _renderItem: function(ul, item) {
    ...
    if (item.category === catSearchTags) {
      if (item.d) {
        ...
        div.html(
          label + '<span class="searchTagHolderResult"> (' + item.h + ")</span>"
        );
      }
    } else {
      div.html(label);
...
function rankMatch(match, category) {
  ...
  for (var i = 1; i < match.length; i++) {
    ...
    if (match[i]) delta += match[i].length;
  }
  if (category === catTypes) {
    // lower ranking if a type name contains unmatched camel-case parts
    if (/[A-Z]/.test(input.substring(matchEnd))) delta += 5;
    if (/[A-Z]/.test(input.substring(0, index))) delta += 5;
  }
    ...
    source: function(request, response) {
      ...
      if (typeSearchIndex) {
        $.each(typeSearchIndex, function(index, item) {
          ...
          var name =
            request.term.indexOf(".") > -1 ? item.p + "." + item.l : item.l;
